...
plt.title('MFCC')
counter=0
fileDirCASIA = r'D:\王晨E\pythonProject\for_test\mysite\static\Role database.1'
mfccs={}
mfccs['成年']=[]
mfccs['未成年']=[]
# mfccs['disgust']=[]
...
for persondir in listdir:
    ...
    with open('mfcc_role.1_feature_dict.pkl', 'wb') as f:
        pickle.dump(mfccs, f)
#读取特征
mfccs={}
with open('mfcc_role.1_feature_dict.pkl', 'rb') as f:
    mfccs=pickle.load(f)
 #设置标签
emotionDict={}
emotionDict['成年']=0
...
with open('mfcc_role.1_model_para_dict.pkl', 'wb') as f:
...
model = Sequential()
model.add(Conv1D(256, 5,padding='same',input_shape=(121,1)))
...
model.summary()
cnnhistory=model.fit(x_train, y_train, batch_size=12, epochs=300, validation_data=(x_val, y_val))
...
plt.show()
model_name = 'role.1_Voice_Detection_Model.h5'
...
loaded_model.load_weights("saved_models/role.1_Voice_Detection_Model.h5")
