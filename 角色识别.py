...
plt.title('MFCC')
counter=0
fileDirCASIA = r'D:\王晨E\pythonProject\for_test\mysite\static\Role database'
mfccs={}
mfccs['成年']=[]
mfccs['未成年1']=[]
mfccs['未成年2']=[]
...
for persondir in listdir:
    ...
    with open('mfcc_role_feature_dict.pkl', 'wb') as f:
        pickle.dump(mfccs, f)
#读取特征
mfccs={}
with open('mfcc_role_feature_dict.pkl', 'rb') as f:
    mfccs=pickle.load(f)
 #设置标签
emotionDict={}
emotionDict['成年']=0
...
data=data+mfccs['成年']
print(len(mfccs['成年']))
for i in range(len(mfccs['成年'])):
    labels.append(0)
data=data+mfccs['未成年1']
print(len(mfccs['未成年1']))
for i in range(len(mfccs['未成年1'])):
    labels.append(1)
print(len(mfccs['未成年2']))
data=data+mfccs['未成年2']
for i in range(len(mfccs['未成年2'])):
    labels.append(2)
print(len(mfccs['未成年3']))
data=data+mfccs['未成年3']
for i in range(len(mfccs['未成年3'])):
    labels.append(3)
print(len(mfccs['未成年4']))
data=data+mfccs['未成年4']
for i in range(len(mfccs['未成年4'])):
    labels.append(4)
print(len(mfccs['未成年5']))
data=data+mfccs['未成年5']
for i in range(len(mfccs['未成年5'])):
    labels.append(5)
print(len(data))
print(len(labels))
#设置数据维度
...
with open('mfcc_role_model_para_dict.pkl', 'wb') as f:
...
model.summary()
cnnhistory=model.fit(x_train, y_train, batch_size=100, epochs=48, validation_data=(x_val, y_val))
...
plt.show()
model_name = 'role_Voice_Detection_Model.h5'
...
loaded_model.load_weights("saved_models/role_Voice_Detection_Model.h5")
